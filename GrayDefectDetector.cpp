...
AlgorithmResult GrayDefectDetector::detect(const cv::Mat& rgb, const GrayDetectParams& params)
{
	...
	// 2. 灰度区间分割（inRange）：
	//    暗缺陷 = 灰度 < params.darkHigh；亮缺陷 = 灰度 > params.brightLow。
	//    背景为中间灰度的场景下，直方图三峰清晰，双区间精确分离亮/暗缺陷。
	cv::Mat maskDark, maskBright;
	cv::inRange(smooth, params.darkLow, params.darkHigh, maskDark);
	cv::inRange(smooth, params.brightLow, params.brightHigh, maskBright);
	...
	auto extractBoxes = [&](const cv::Mat& mask, const char* label) {
		...
		const double maxBoxArea = rgb.cols * rgb.rows * params.maxBoxRatio;

		int count = 0;
		for (const auto& c : contours)
		{
			if (cv::contourArea(c) < params.minContourArea)
				continue;
			...
			double conf = std::abs(dev - bg) / (params.confFullGain * 0.5) + 0.2;
